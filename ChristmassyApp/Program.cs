...
namespace ChristmassyApp {
    class Program {
        private static async Task<AnalysisResult> UploadAndAnalyzeImage(string imageFilePath) {
            VisionServiceClient VisionServiceClient = new VisionServiceClient("YOUR KEY GOES HERE", "HOST SERVER GOES HERE");
            ...
            Task.Run(async () => {
                ...
                foreach (var image in imagesToAnalyse) {
                    ...
                    if (myResult.Tags.Length != 0) {
                        System.IO.Directory.CreateDirectory(myResult.Tags[0].Name);
                        int mostConfidentIndex = 0;
                        for (int i = 0; i < myResult.Tags.Length; i++) {
                            if (myResult.Tags[i].Confidence > myResult.Tags[mostConfidentIndex].Confidence)
                                mostConfidentIndex = i;
                        }
                        destinationPath = myResult.Tags[mostConfidentIndex].Name + image.Substring(1);
                    } else {
                        System.IO.Directory.CreateDirectory("tagless");
                        destinationPath = "tagless" + image.Substring(1);
                    }
